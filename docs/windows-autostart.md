...
Run the service silently in the background on every login, without a terminal
window or manual start.

## 1. Clone the repo somewhere permanent

Replace `<path-to-clone>` below with wherever you want the repo to live (e.g.
`C:\watermark-remover` or `C:\Users\<you>\watermark-remover`). Use the same
...
WshShell.Run "python skills\remove-ai-marks\scripts\server.py --host 127.0.0.1 --port 8765", 0, False
```

## 3. Register a scheduled task

This does **not** require Administrator privileges — `-AtLogOn` with a
user-level trigger runs under your own account, so a regular PowerShell window
is enough.
...
Register-ScheduledTask -TaskName "WatermarksRemoverService" -Action $action -Trigger $trigger -Settings $settings -Description "Auto-starts the watermark-remover HTTP service at login"
```
