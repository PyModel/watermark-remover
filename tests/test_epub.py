...
from test_clean_image import _minimal_png_with_text
...
def test_clean_epub_skips_encrypted_parts():
    ...
    epub = _build_epub(body_text="", encrypted_parts=("OEBPS/chapter1.xhtml",))
    buf = io.BytesIO()
    with (
    ...
    with zipfile.ZipFile(io.BytesIO(cleaned)) as zf:
        assert zf.read("OEBPS/chapter1.xhtml") == ciphertext
